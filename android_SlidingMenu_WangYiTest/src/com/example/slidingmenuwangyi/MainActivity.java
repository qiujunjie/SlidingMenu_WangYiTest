...
public class MainActivity extends SlidingFragmentActivity implements
	...
	FindPeopleFragment peopleFragment = null;
	...
	public void selectItem(int position, String title) {
		...
		FragmentTransaction beginTransaction = fragmentManager
				.beginTransaction();
		hid(beginTransaction);
		// if(mList.size() != 0){
		// for (Fragment ff : mList) {
		// beginTransaction.hide(ff).commit();
		// }
		// }
		...
		case 0:
			...
			// doAdd(homeFragment, beginTransaction);
			break;
		case 1:
			if (null == peopleFragment) {
				peopleFragment = new FindPeopleFragment();
				beginTransaction.add(R.id.content, peopleFragment);
			} else {
				beginTransaction.show(peopleFragment);
			}
			// doAdd(peopleFragment, beginTransaction);
			break;
		case 2:
	...
	private void hid(FragmentTransaction beginTransaction) {
		if (homeFragment != null) {
			beginTransaction.hide(homeFragment);
		}
		if (peopleFragment != null) {
			beginTransaction.hide(peopleFragment);
		}
	}
}
